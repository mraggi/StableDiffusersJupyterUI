...
class UnifiedStableDiffusionPipeline(DiffusionPipeline):
    ...
    def img2img(
        ...
        init_latent_dist = self.vae.encode(img.to(self.device)).latent_dist
        ...
        init_latents = init_latents.detach().clone().cpu()

        t_start = max(steps - init_timestep + offset, 0)
        for i, t in enumerate(progress_bar(self.scheduler.timesteps[t_start:], parent=mb)):
            t_index = t_start + i
        ...
        return {'sample': image, 'latents': init_latents}
    ...
    def inpaint(
        ...
        #if mask_fill == "Noise":
            #img = img*(1-mask) + torch.rand_like(img)*mask
        #elif mask_fill == "Black":
            #img = img*(1-mask) + torch.zeros_like(img)*mask
